...
@section('content')
    ...
    <div class="col-md-10">
        ...
        <div class="row">
            <div class="col-md-12">
                <form class="form-inline" action="{{ route('documents.news.index') }}">
        ...
        <div class="row">
            <div class="col-md-12">
                <a href="{{ route('documents.news.add') }}" class="btn btn-success btn-sm">
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                @foreach($items as $item)
                                    <tr>
                                        <td>{{ $item['guid'] }}</td>
                                        <td><a title="{{$item['title']}}" href="{{ route('documents.news.edit', $item['_id']) }}" target="_blank">{{ \App\Helpers\StringHelper::subtitle_cn($item['title'], 58) }}</a></td>
                                        <td>{{ $item['promulgation_date'] }}</td>
                                        <td>
                                            <a href="{{ route('documents.news.edit', $item['_id']) }}" target="_blank">
                                                <button class="btn btn-sm btn-primary">
                                                    编辑
                                                </button>
                                            </a>
        ...
        @include('layouts.pagination', ['routeName'=>'documents.news.index', 'count'=>$itemsCount, 'page'=>$page_num, 'size' => $pageSize, 'filters' => $filters])
    </div>
@endsection
